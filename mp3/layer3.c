...
frame->overlap = malloc(sizeof(mad_fixed_t)*2*32*18);
memset(frame->overlap,0,sizeof(mad_fixed_t)*2*32*18);
...
# endif
  ...
  if (frame_free >= next_md_begin) {
	...
	memcpy(tmpbuf, stream->main_data + stream->md_len - extra, extra);
	memcpy(*stream->main_data, tmpbuf, extra);
	stream->md_len = extra;
      }
    }
    else
